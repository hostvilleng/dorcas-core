...
use App\Transformers\PayrollAuthorityTransformer;
use Carbon\Carbon;
...
class Authority extends Controller
{
    ...
    public function create( Request $request, Manager $fractal){
        ...
        $authority = $company->PayrollAuthority()->create($data);
        $resource = new Item($authority, new PayrollAuthorityTransformer());
    ...
    public function search(Request $request , Manager $fractal){
        ...
        $paginator = $company->PayrollAuthority()->when($search, function ($query) use ($search) {
            return $query->where('authority_name', 'like', '%'.$search.'%');
        })
            ->oldest('authority_name')
            ->paginate($limit);
        $resource = new Collection($paginator->getCollection(), (new PayrollAuthorityTransformer()));
        # create the resource
        if (!empty($search)) {
    ...
    public function single(Request $request, Manager $fractal, string $id)
    {
        $company = $this->company($request);
        # get the company
        $authority = $company->PayrollAuthority()->where('uuid', $id)->firstOrFail();
        # get the account
        $resource = new Item($authority, new PayrollAuthorityTransformer(),'payrollAuthority');
    ...
    public function delete(Request $request, Manager $fractal, string $id)
    {
        ...
        if (!(clone $authority)->delete()) {
            throw new DeletingFailedException(
                'Errors while deleting the specified Payroll Authority. Please try again.'
            );
        }
        $transformer = new PayrollAuthorityTransformer();
        $transformer->setDefaultIncludes([]);
        # adjust the default includes
        $resource = new Item($authority, $transformer, 'payrollAuthority');
        return response()->json($fractal->createData($resource)->toArray(), 200);
    }
