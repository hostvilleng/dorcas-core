...
class States extends Controller
{
    ...
    public function index(Request $request, Manager $fractal)
    {
        ...
        if ($request->has('country_id')) {
            $country = Country::where('uuid', $request->input('country_id'))->first();
        }
        if (empty($search)) {
            # no search parameter
            $paginator = \App\Models\State::with('country')->when($country, function ($query) use ($country) {
            ...
            $builder = \App\Models\State::search($search);
            if (!empty($country)) {
                $builder->where('country_id', $country->id);
            }
            $paginator = $builder->orderBy('name')->paginate($limit);
        }
        # get the states
        $resource = new Collection($paginator->getCollection(), new StateTransformer(), 'state');
        # create the resource
        if (!empty($search)) {
    ...
    {
        $company = \App\Models\State::where('uuid', $id)->firstOrFail();
        # retrieve the company
        $resource = new Item($company, new StateTransformer(), 'state');
        # get the resource
        return response()->json($fractal->createData($resource)->toArray());
    }
}